...
const char* STATUS_URL = "https://cc-library-dashboard-ecegewg6bqfracfd.austriaeast-01.azurewebsites.net/api/sensor/status";
...
constexpr float DETECT_DISTANCE_CM = 30.0f;  // computed detection distance

// Timing and buffer
//const unsigned long MIN_TIME_DIFF_US = 300000UL;       // min separation between valid records in microseconds (1 s)

const unsigned long MIN_TIME_DIFF_MS = 1000UL;  // 1 second
...
const unsigned long WORKING_STATUS_INTERVAL_MS = 30UL * 60UL * 1000UL;
unsigned long lastWorkingStatusPost = 0;
...
bool uploadSingleRecord(const Record& r) {
  ...
  int n = snprintf(
    tmp,
    sizeof(tmp),
    "{\"signal\":%d,\"sensorId\":\"%s\",\"recorded_at\":\"%s\"}",
    SIGNAL_VALUE,
    SENSOR_ID,
    timestr
  );
...
const char* EAP_USERNAME = "s1155214508@link.cuhk.edu.hk";
const char* EAP_PASSWORD = "UiSUFNJFe5NCLxL";

// ======= Testing WIFI =======
const char* SSID = "POCOF7";
const char* WIFI_PASSWORD = "e6bq2qvbuq";
const bool WIFI_FLAG = 1;





bool uploadSensorStatus(const char* status) {
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("Cannot upload sensor status: WiFi disconnected.");
    return false;
  }
  ...
  if (!http.begin(STATUS_URL)) {
    Serial.println("Status upload: HTTP begin failed.");
    return false;
  }
  ...
  Serial.println("Posting sensor status:");
  Serial.println(payload);
  ...
  if (success) {
    Serial.printf(
      "Sensor status uploaded: %s (%d)\n",
      status,
      httpCode
    );
  } else {
    Serial.printf(
      "Sensor status upload failed: %d, %s\n",
      httpCode,
...
void sleepUntilNextStart(uint64_t secondsToSleep) {
  Serial.println("Sending sleeping status...");

  uploadSensorStatus("sleeping");

  delay(300);

  esp_sleep_enable_timer_wakeup(
    secondsToSleep * uS_TO_S_FACTOR
  );
...
void setup() {
  ...
  // TEMPORARY: HC-SR04 sensor test only
/*
  Serial.println("TEST MODE: skipping WiFi and time sync");
  activeWindow = true;
...
*/
  ...
  // Use three time servers for better reliability.
  configTime(
    8 * 3600,
    0,
    "pool.ntp.org",
  ...
  bool firstNtpFailureReported = false;

  // Stay awake and keep retrying until time becomes valid.
  while (time(nullptr) < 1609459200UL) {
    Serial.println("Waiting for NTP time...");

    unsigned long ntpStart = millis();

    // One NTP attempt may take up to 30 seconds.
    while (time(nullptr) < 1609459200UL &&
          millis() - ntpStart < 30000) {
      delay(200);
      Serial.print(".");
    }
    ...
    // NTP worked: leave the retry loop.
    if (time(nullptr) >= 1609459200UL) {
      break;
    }

    // This runs only if the current NTP attempt failed.
    Serial.println("NTP attempt failed.");

    // Post the failed-NTP status only once.
    if (!firstNtpFailureReported) {
      Serial.println("Posting status: ntp_failed");

      bool statusPosted = uploadSensorStatus("ntp_failed");

      if (statusPosted) {
        Serial.println("NTP failure status posted successfully.");
      } else {
        Serial.println("Could not post NTP failure status.");
      }

      firstNtpFailureReported = true;
    }

    Serial.println("Retrying NTP in 5 seconds...");
    delay(5000);

    // Begin a new NTP request.
    configTime(
      8 * 3600,
      0,
      "pool.ntp.org",
  ...
  Serial.print("Raw Unix time: ");
  Serial.println((long long)time(nullptr));
  ...
  if (!(hour >= START_HOUR && hour < END_HOUR)) {
    ...
    sleepUntilNextStart(secs);
  }

  activeWindow = true;
  lastUploadAttempt = millis() - UPLOAD_INTERVAL_MS;

  // Immediately show that the sensor became active.
  uploadSensorStatus("working");

  // Start the 30-minute countdown after the first working post.
  lastWorkingStatusPost = millis();
...
void loop() {
  ...
  unsigned long timeout_us = 10000UL;  // 10 ms，約 170 cm 最大距離. (unsigned long)(MAX_DISTANCE_CM * 58.0f) + 120000UL;  // add margin with recommended margin 60ms
    ...
    if (distance_cm <= DETECT_DISTANCE_CM) {
      ...
      if (acceptedBySpacing) {
        ...
        if (WiFi.status() != WL_CONNECTED) {
          ...
          while (WiFi.status() != WL_CONNECTED && millis() - s2 < 15000) {
            delay(100);
          }
        }
  ...
  // Post status=working every 30 minutes while active.
  if (now_ms - lastWorkingStatusPost >= WORKING_STATUS_INTERVAL_MS) {
    if (WiFi.status() == WL_CONNECTED) {
      Serial.println("30-minute status check: posting working status.");

      if (uploadSensorStatus("working")) {
        lastWorkingStatusPost = now_ms;
        Serial.println("30-minute working status posted.");
      } else {
        Serial.println("Working status POST failed. Will retry in 1 minute.");
        lastWorkingStatusPost =
          now_ms - WORKING_STATUS_INTERVAL_MS + 60000UL;
      }

    } else {
      Serial.println("30-minute status check skipped: WiFi is disconnected.");

      // Try status again after 1 minute instead of waiting a full 30 minutes.
      lastWorkingStatusPost =
        now_ms - WORKING_STATUS_INTERVAL_MS + 60000UL;
    }
  }
